...
class ExpressionEvaluator(cursor: ACursor) {
  ...
  def evaluateJson(mapping: Expression[_]): Either[DecodingFailure, Json] =
    mapping match {
      case Conditional(value, when, defaultTo) =>
        evaluateWhen(when).flatMap(unfiltered =>
          if (unfiltered) evaluateJson(value)
          else defaultTo.traverse(evaluateJson).map(_.getOrElse(Json.Null)),
        )

      case jsonPath: JsonPath => Right(jsonPath.evaluate(cursor).toJson)

      case const @ Const(_) => Right(const.asJson.value)

      case Expressions(expressions) => expressions.traverse(evaluateJson).map(results => Json.arr(results: _*))
    }
